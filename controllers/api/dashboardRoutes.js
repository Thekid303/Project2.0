...
router.get('/', withAuth, async (req, res) => {
    ...
    try {
        ...
        res.render('all-project', {     // Then, the 'all-project' handlebar template is rendered 
...
router.get("./project/:id", async (req, res) => {
    ...
    try {
      ...
      if (postData) { //if postData is true (exists)
        ...
        res.render("employee-project", {   // Then, the 'employee-project' handlebar template is rendered     
...
//Rest Arc Patter
  try {
    ...
    const postData = await Project.findByPk(req.params.id, { //model interacts with the DB same as DB SELECT * FROM
        //
      include: [
        { model: User},
        { model: Comment },
    ...
    // We use .get({ plain: true }) on the object (postData) to serialize it so that it only includes the data that we need. 
      ...
      res.render("single-project", {      // Then, the 'single-project' handlebar template is rendered    
          projectData,                    // and the projectData information is passed into the  
          layout: 'dashboard',            // dashboard handlebar template
        });
...
router.post('/new-project', withAuth, (req, res) => {
    res.render('new-project', {     // Then, the 'new-project' handlebar template is rendered on the   
